 ...
 * @author Manuel Bua
 * @author Jens Vossnack
 */
public class CustomAtlasTmxMapLoader extends BaseTmxMapLoader<AtlasTmxMapLoader.AtlasTiledMapLoaderParameters> {

	protected Array<Texture> trackedTextures = new Array<Texture>();
        
        public HashMap<Integer, MapObject[]> tilesetCollisionObjects;
        public HashMap<Integer, WorldLightProperties> tilesetLights;
	...
	protected TiledMap loadMap (Element root, FileHandle tmxFile, AtlasResolver resolver) {
		TiledMap map = new TiledMap();
                ...
                tilesetCollisionObjects = new HashMap<Integer, MapObject[]>();
                tilesetLights = new HashMap<Integer, WorldLightProperties>();
	...
	protected void loadTileset (TiledMap map, Element element, FileHandle tmxFile, AtlasResolver resolver) {
				...
				if (tile != null) {
					...
					if (probability != null) {
						tile.getProperties().put("probability", probability);
                                        ...
                                        Element tileobjectgroup = tileElement.getChildByName("objectgroup");
                                        if(tileobjectgroup != null){
                                            Element properties = tileobjectgroup.getChildByName("properties");
                                            if (properties != null) {
                                                    loadProperties(tile.getProperties(), properties);
                                            }
                                            
                                            this.tilesetCollisionObjects.put(tile.getId(), getTileObjects(tileobjectgroup.getChildrenByName("object")));
                                        
                                            //Element objectGroupProperties = tileobjectgroup
                                            if(tile.getProperties().get("light") != null){
                                                Boolean isLight = tile.getProperties().get("light", Boolean.class);
                                                if(isLight)
                                                {
                                                    this.tilesetLights.put(tile.getId(), new WorldLightProperties());
                                                }
                                            }
                                        }
        ...
        protected MapObject[] getTileObjects(Array<Element> tileObjects) {
            int objectCount = tileObjects.size;
            MapObject[] retval = new MapObject[objectCount];
            
            for(int objIdx = 0; objIdx < objectCount; objIdx++){
                Element element = tileObjects.get(objIdx);
                MapObject object = null;
                float scaleX = convertObjectToTileSpace ? 1.0f / mapTileWidth : 1.0f;
                float scaleY = convertObjectToTileSpace ? 1.0f / mapTileHeight : 1.0f;

                float x = element.getFloatAttribute("x", 0) * scaleX;
                float y = element.getFloatAttribute("y", 0) * scaleY;
                ...
                if (element.getChildCount() > 0) {
                                    ...
                                    if ((child = element.getChildByName("polygon")) != null) {
                                            ...
                                            for (int i = 0; i < points.length; i++) {
                                                    String[] point = points[i].split(",");
                                                    vertices[i * 2] = (x + Float.parseFloat(point[0])) * scaleX;
                                                    vertices[i * 2 + 1] = (y + Float.parseFloat(point[1])) * scaleY * (flipY ? -1 : 1);
                                            }
